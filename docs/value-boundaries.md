...
`runtime.read` and `Readable.current()` return current values, including during a batch. Derived reads can advance necessary computation; notifications remain deferred. Returned collection views are durable version snapshots with owner disposal checks. Readonly payload ownership remains unchanged: compute a replacement and call `runtime.update`, or use the existing borrowed collection draft. There is no deep-proxy rollback for illegal payload mutation. Only drafts and processor context readers have callback-bound lifetimes; public current reads have no borrowed batch reader.
