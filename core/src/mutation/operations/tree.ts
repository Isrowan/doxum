...
const positionIndex = (index: number | undefined, length: number, at: DocumentAddress): number => {
  if (index !== undefined && (!Number.isInteger(index) || index < 0))
    return issue.fail(at, 'invalid-tree-index', 'Tree index must be nonnegative.');
...
export const insert = (
  ...
  if (typeof id !== 'string') return issue.fail(at, 'invalid-key', 'Tree keys must be strings.');
  if (treeTopology.contains(tree, id))
    return issue.fail(at, 'duplicate-tree-node', 'Tree node already exists.');
  ...
  if (
    ...
    return issue.fail(at, 'missing-tree-parent', 'A nonempty tree requires an existing parent.');
...
export const replace = (
  ...
  if (!entry) return issue.fail(at, 'missing-tree-node', 'Tree node does not exist.');
...
export const move = (
  ...
  if (
    ...
    parentId === undefined ||
    !treeTopology.contains(tree, parentId)
  )
    return issue.fail(
      at,
      'missing-tree-parent',
  ...
  for (
    ...
    if (ancestor === id)
      return issue.fail(at, 'tree-cycle', 'Cannot move a node under its descendant.');
